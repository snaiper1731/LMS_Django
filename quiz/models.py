...
CHOICE_ORDER_OPTIONS = (
    ...
    ('random', _('Случайно')),
    ('none', _('По добавлению'))
)

CATEGORY_OPTIONS = (
    ('exam', _('Экзамен')),
    ('practice', _('Тренировачный'))
)
...
class Quiz(models.Model):
    ...
    random_order = models.BooleanField(blank=False, default=False, verbose_name=_("Случайный порядок"),
        help_text=_("Отображать вопросы в случайном порядке?"))

    answers_at_end = models.BooleanField(blank=False, default=False, verbose_name=_("Показывать ответы в конце?"),
        help_text=_("Правильный ответ НЕ отображается после вопроса. Ответы отображаются в конце."))

    exam_paper = models.BooleanField(blank=False, default=True, verbose_name=_("Сохранять результат"),
        help_text=_("Если да, результат каждой попытки пользователя будет сохранен."))

    single_attempt = models.BooleanField(blank=False, default=False, verbose_name=_("Одна попытка"),
    ...
    pass_mark = models.SmallIntegerField(blank=True, default=50, verbose_name=_("Проходной балл"), validators=[MaxValueValidator(100)],
        help_text=_("Процент необходимый для сдачи теста"))
...
class Progress(models.Model):
    ...
    score = models.CharField(max_length=1024, verbose_name=_("Балл"), validators=[validate_comma_separated_integer_list])
...
class Sitting(models.Model):
    ...
    question_list = models.CharField(max_length=1024, verbose_name=_("Список вопросов"),
        validators=[validate_comma_separated_integer_list])

    incorrect_questions = models.CharField(max_length=1024, blank=True, verbose_name=_("Неправильные вопросы"),
        validators=[validate_comma_separated_integer_list])

    current_score = models.IntegerField(verbose_name=_("Текущие баллы"))
    complete = models.BooleanField(default=False, blank=False, verbose_name=_("Тест завершен"))
    user_answers = models.TextField(blank=True, default='{}', verbose_name=_("Ответ пользователя"))
...
class Question(models.Model):
    ...
    figure = models.ImageField(upload_to='загружено/%Y/%m/%d', blank=True, null=True, verbose_name=_("Рисунок"))
    content = models.CharField(max_length=1000, blank=False, 
        help_text=_("Введите текст вопроса"), verbose_name=_('Вопрос'))
    explanation = models.TextField(max_length=2000, blank=True,
        help_text=_("Объяснение правильного ответа"),
        verbose_name=_('Обьяснение'))
...
class MCQuestion(Question):
    ...
    choice_order = models.CharField(
        ...
        help_text=_("Порядок, в котором отображаются варианты с несколькими ответами пользователю"),
...
class Choice(models.Model):
    question = models.ForeignKey(MCQuestion, verbose_name=_("Тест"), on_delete=models.CASCADE)

    choice = models.CharField(max_length=1000, blank=False,
        help_text=_("Введите текст"),
